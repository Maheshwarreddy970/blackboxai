...
export default function BlogCard({ href, date, image, title, subtitle }: { href: string, date: string, image: string, title: string, subtitle: string }) {
  return (
    <Link href={href} className='  relative '>
      <p className=' text-[0.73rem] absolute -top-5 right-4 bg-black/60 p-1 rounded-lg z-10 text-right'>{date}</p>
      ...
      <div className=' flex  mt-16 justify-between group  '>
        <p className=' group-hover:border-b font-medium '>
          {title}
        </p>
